...
for record = RECORDS
    ...
    if (~isfile(sprintf('%sm.mat', record)))
        ...
        cmd=sprintf('docker run -it --rm -v "$(pwd)":/data ghcr.io/spagnolog/wfdb-docker:main wfdb2mat -r %s', record);
        system(cmd);
    end
    ...
    cmd = sprintf('docker run -it --rm -v "$(pwd)":/data ghcr.io/spagnolog/wfdb-docker:main wrann -r %s -a qrs <%s.asc', record, record);
    system(cmd);
    ...
    cmd = sprintf('docker run -it --rm -v "$(pwd)":/data ghcr.io/spagnolog/wfdb-docker:main bxb -r %s -a atr qrs -l /data/eval1.txt /data/eval2.txt -f 0', record);
    [status, cmdout] = system(cmd);
end

% After the run, calculate the aggregated statistics, save the results into results.txt
system('docker run -it --rm -v "$(pwd)":/data ghcr.io/spagnolog/wfdb-docker:main sumstats eval1.txt eval2.txt >results.txt');
